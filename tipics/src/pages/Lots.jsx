...
function BarraDistribucio({ lot }) {
  ...
  if (total === 0 && lot.kg_inicials) {
    ...
    return (
      <div>
        ...
        <div style={{ background: 'var(--c-cream)', borderRadius: 4, height: 8, overflow: 'hidden' }}>
          <div style={{ width: `${pct}%`, background: '#956C58', height: '100%', transition: 'width 0.4s' }} />
        </div>
      </div>
    )
  }
  ...
  const segments = [
    { val: bot, color: '#212322', label: 'Bot.' },
    { val: ven, color: '#2A5A3A', label: 'Ven.' },
    { val: mos, color: '#7A4F1A', label: 'Mos.' },
    { val: mer, color: '#8B2A2A', label: 'Mer.' },
  ].filter(s => s.val > 0)
...
export default function Lots() {
  ...
  const grouped = productos
    .map(p => ({ prod: p, lots: lots.filter(l => l.producto_id === p.id) }))
    .filter(g => g.lots.length > 0)

  const ESTAT_BADGE = { actiu: 'badge-green', esgotat: 'badge-gray', retirat: 'badge-red', caducat: 'badge-red' }
  const tipusLabel = { entrada_kg: '📦 Entrada kg', entrada_ud: '📦 Entrada ud', conversio_ud: '⚖️ Conversió', enviament: '🚚 Enviament', venda: '💰 Venda', mostra: '🎁 Mostra', merma: '⚠️ Merma', devolucio: '↩️ Devolució' }
  const tipusColor = { entrada_kg: '#956C58', entrada_ud: '#956C58', conversio_ud: '#6B4F3A', enviament: '#212322', venda: '#2A5A3A', mostra: '#7A4F1A', merma: '#8B2A2A', devolucio: '#4A3F8F' }
          ...
          <div key={prod.id}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, paddingBottom: 8, borderBottom: '1px solid var(--c-border)' }}>
              ...
              <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--c-text-light)' }}>
                {prodLots.filter(l => l.estat === 'actiu').length} actius · {prodLots.length} total
              </span>
            </div>
