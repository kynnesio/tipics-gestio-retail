...
const empty = { nombre: '', direccion: '', zona: '', contacto_nombre: '', contacto_email: '', contacto_telefono: '', alquiler_fijo_mensual: '0', comision_variable_pct: '10', activa: true }
...
export default function Tiendas() {
  ...
  const openEdit = (r) => { setForm({ ...r, alquiler_fijo_mensual: String(r.alquiler_fijo_mensual), comision_variable_pct: String(r.comision_variable_pct) }); setEditId(r.id); setModal(true) }
      ...
      {loading ? <div style={{ color: '#6b6b68' }}>Carregant...</div> : (
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <table>
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: 'center', color: '#6b6b68', padding: '2rem' }}>Cap botiga registrada. Afegeix les teves 15 botigues!</td></tr>
              )}
              {rows.map(r => (
                <tr key={r.id}>
                  <td>
        ...
        <div className="modal-overlay" onClick={() => setModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="form-group"><label>Adreça</label><input value={form.direccion} onChange={f('direccion')} /></div>
